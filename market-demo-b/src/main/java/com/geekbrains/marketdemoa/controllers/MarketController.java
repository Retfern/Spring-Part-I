package com.geekbrains.marketdemoa.controllers;

import com.geekbrains.marketdemoa.entites.Basket;
import com.geekbrains.marketdemoa.entites.Category;
import com.geekbrains.marketdemoa.entites.Item;
import com.geekbrains.marketdemoa.repositories.specifications.ItemSpecifications;
import com.geekbrains.marketdemoa.services.CategoryService;
import com.geekbrains.marketdemoa.services.ItemService;
import com.geekbrains.marketdemoa.utils.ItemFilter;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.*;

import java.util.Arrays;
import java.util.List;
import java.util.Map;
import java.util.Set;


@Controller
public class MarketController {
    private ItemService itemService;
    private CategoryService categoryService;

    @Autowired
    public MarketController(ItemService itemService, CategoryService categoryService) {
        this.itemService = itemService;
        this.categoryService = categoryService;
    }

    @GetMapping("/")
    public String index(Model model, @RequestParam Map<String, String> params) {
        int pageIndex = 0;
        if (params.containsKey("p")) {
            pageIndex = Integer.parseInt(params.get("p")) - 1;
        }
        if (params.containsKey("company")) {
            System.out.println(params.containsKey("company"));
        }
        ItemFilter itemFilter = new ItemFilter(params);
        Pageable pageRequest = PageRequest.of(pageIndex, 10, itemFilter.getSortDirection(), itemFilter.getSortField() );
        Page<Item> page = itemService.findAll(itemFilter.getSpec(), pageRequest);
        model = itemService.modelInit(model);
        List<Category> categories = categoryService.getAll();
        model.addAttribute("filtersDef", itemFilter.getFilterDefinition());
        model.addAttribute("categories", categories);
        model.addAttribute("page", page);
        model.addAttribute("basket", itemService.getBasket().getItemList());

        return "index";
    }

    @GetMapping("/edit/{id}")
    public String editItemForm(Model model, @PathVariable Long id) {
        Item item = itemService.findById(id);
        List<Category> categories = categoryService.getAll();
        model.addAttribute("item", item);
        model.addAttribute("categories", categories);
        return "edit_item";
    }

    @GetMapping("/basket/")
    public String addItemInBasket(@RequestParam (value = "id") Long id, @RequestParam(value = "sum") Integer sum) {
        System.out.println("ID= " +id + " sum= " +sum);
        itemService.getBasket().addItem(id, sum);
        return "redirect:/";
    }

    @GetMapping("/basket_update/")
    public String addItemInBasketUpdate(@RequestParam (value = "id") Long id, @RequestParam(value = "sum") Integer sum) {
        System.out.println("ID= " +id + " sum= " +sum);
        itemService.getBasket().addItem(id, sum);
        return "redirect:/basket_view";
    }

    @PostMapping("/edit")
    public String saveItem(@ModelAttribute(name = "item") Item item) {
        itemService.save(item);
        return "redirect:/";
    }

    @GetMapping("/basket_view")
    public String getBasket(Model model) {
        System.out.println("basket_view start");
        model.addAttribute("basket", itemService.getBasket().getItemList());
        model.addAttribute("priceOrder", itemService.getBasket().getPriceOrder());
        return "basket_v";
    }

    @GetMapping("/basket_delete/{id}")
    public String deleteItemFromBasket(Model model, @PathVariable Long id) {
        itemService.getBasket().deleteItem(id);
        return "redirect:/basket_view";
    }

    @GetMapping("/order_pay")
    public String getOrder(Model model) {

        return "redirect:/basket_view";
    }

    @GetMapping("/multi_field_result")
    @ResponseBody
    public String processMultiFieldForm(@RequestParam String[] prog_langs) {
        System.out.println("/multi_field_result start!!!");
        return Arrays.toString(prog_langs);
    }
}